...
function toPersistedRow(row: RawRow): Omit<PersistedSignalRow, "linkedPsychologistIds"> {
  return {
    id: row.id,
...
export const crisisFeedRepository = {
  async findActiveByPsychologistAndType(
    ...
    return rows.map((r) => ({
      ...toPersistedRow(r),
      linkedPsychologistIds: [psychologistId],
    }));
  },
  ...
  ): Promise<{ items: PersistedSignalRow[]; total: number }> {
  ...
  async resolveSignal(
    ...
    const [withStudent] = await db
      ...
      .limit(1);
    return {
      ...toPersistedRow(withStudent),
      linkedPsychologistIds: [input.resolvedBy],
    };
  },
